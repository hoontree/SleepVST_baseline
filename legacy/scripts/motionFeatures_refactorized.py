...
class MotionFeatureExtractor:
    ...
    def __init__(self, target_fps: int = 4, out_size: Tuple[int, int] = (224, 224)):
        self.target_fps = target_fps
        self.out_size = tuple(out_size)
        self.pts_src = np.array([[197, 63], [507, 63], [611, 426], [102, 420]], dtype=np.float32)
        
    def scale_quad(self, pts: np.ndarray, scale: float = 1.2) -> np.ndarray:
    ...
    def homography_transform(self, img: np.ndarray) -> np.ndarray:
        ...
        pts_dst = np.array([[0, 0], [self.out_size[0], 0], 
                          ...
                          dtype=np.float32)
        pts_src_scaled = self.scale_quad(self.pts_src, scale=1.2)
    ...
    def compute_motion_features(self, v_seq: np.ndarray, s_seq: np.ndarray, 
                              seconds: List[int] = [30, 300], 
                              thresholds: List[float] = [0.01, 0.1, 1.0]) -> Dict[str, np.ndarray]:
...
def process_single_video(video_info: Tuple[str, str, bool, int, Tuple[int, int]]) -> Optional[str]:
    ...
    try:
        ...
        for region in ['Head', 'Body', 'Outer']:
            ...
            features = extractor.compute_motion_features(v_seq, s_seq)
...
def process_videos_batch(video_files: List[str], out_dir: str, skip_existing: bool,
                        target_fps: int, out_size: Tuple[int, int], num_workers: int,
                        batch_size: int) -> List[str]:
    """Process videos in batches using multiprocessing."""
    results = []
    ...
    for i in range(0, total_videos, batch_size):
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            with tqdm(total=len(batch), 
                     desc=f"Batch {batch_num}/{total_batches}") as pbar:
