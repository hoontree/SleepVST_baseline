...
def process_file(hw_path, seq_len, xml_dir, step=10):
    ...
    try:        
        ...
        # xml 라벨 파일 경로 생성
        if 'shhs1' in hw_path:
            label_path = path.join(xml_dir, 'shhs1', f'{basename}-nsrr.xml')
        elif 'shhs2' in hw_path:
            label_path = path.join(xml_dir, 'shhs2', f'{basename}-nsrr.xml')
        # 필요한 파일이 모두 존재하는지 확인
        ...
        if self.split == 'train' or self.split == 'val':
            ...
            for hw_file in tqdm(hw_files, desc=f'테스트 데이터 로딩'):
                ...
                if 'shhs1' in basename:
                    label_path = path.join(self.xml_dir, 'shhs1', f'{basename}-nsrr.xml')
                else:
                    label_path = path.join(self.xml_dir, 'shhs2', f'{basename}-nsrr.xml')
                epochs = parse_xml(label_path)
                labels = np.array([e['label'] for e in epochs])
