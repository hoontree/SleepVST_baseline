...
def extract_signal(edf_path, dataset_type='SHHS', channels=None, patch_size={'ECG': 300, 'THOR RES': 150, 'EKG': 300, 'Thor': 150}):
    """
    ...
    try:
        ...
        if os.path.exists(xml_path):
            ...
            duration_sec = None
            logger.warning(f"Warning: {os.path.basename(edf_path)}의 XML 파일을 찾을 수 없습니다: {xml_path}")
    except Exception as e:
        duration_sec = None
        logger.error(f"Error: {os.path.basename(edf_path)}의 XML 파일 처리 중 오류 발생: {str(e)}")
    
    # 기존 mne 설정 임시 저장
    original_log_level = logging.getLogger('mne').level
    ...
    try:
        ...
        if dataset_type.upper() == 'SHHS':
            # 필요한 채널이 2개 있는지 확인
            if len(data) < 2:
                raise ValueError(f"SHHS 데이터셋에는 ECG와 THOR RES 두 채널이 필요합니다. 발견된 채널: {channels_to_pick}")
            
            # 채널 맵을 사용하여 올바른 데이터 할당
            ecg_idx = channel_names.index('ECG') if 'ECG' in channel_names else None
            resp_idx = channel_names.index('THOR RES') if 'THOR RES' in channel_names else None
            
            if ecg_idx is None or resp_idx is None:
                raise ValueError(f"필요한 채널이 없습니다. 발견된 채널: {channel_names}")
            
            ecg_signal = data[ecg_idx]  # ECG
            resp_signal = data[resp_idx]  # THOR RES
            ...
            hw = patchify(hw, patch_size['ECG'], step=patch_size['ECG'])
            bw = patchify(bw, patch_size['THOR RES'], step=patch_size['THOR RES'])
        
        else:  # MESA
            # 필요한 채널이 2개 있는지 확인
            if len(data) < 2:
                raise ValueError(f"MESA 데이터셋에는 EKG와 Thor 두 채널이 필요합니다. 발견된 채널: {channels_to_pick}")
            
            # 채널 맵을 사용하여 올바른 데이터 할당
            ecg_idx = channel_names.index('EKG') if 'EKG' in channel_names else None
            resp_idx = channel_names.index('Thor') if 'Thor' in channel_names else None
            ...
            hw = patchify(hw, patch_size['EKG'], step=patch_size['EKG'])
            bw = patchify(bw, patch_size['Thor'], step=patch_size['Thor'])
...
def process_file(edf_file, save_dir, dataset_type):
    ...
    try:
        ...
        # 신호 추출 및 처리
        hw, bw = extract_signal(edf_file, dataset_type=dataset_type, channels=channels)
...
def main():    
    ...
    for dataset in ['shhs1', 'shhs2', 'mesa']:
        if 'shhs' in dataset:
            edf_dir = os.path.join(args.edf_dir_shhs, dataset)
            dataset_type = 'SHHS'
            save_dir = os.path.join(args.save_dir, 'shhs')
        else:
            edf_dir = args.edf_dir_mesa
            dataset_type = 'MESA'
            save_dir = os.path.join(args.save_dir, 'mesa')
        
        os.makedirs(save_dir, exist_ok=True)

        edf_files = glob(os.path.join(edf_dir, '*.edf'))
        logger.info(f'데이터셋 {dataset} ({dataset_type} 타입) 처리 시작 (총 {len(edf_files)} 파일)')
        print(f'Processing {dataset} ({dataset_type} type, {len(edf_files)} files)...')
                    ...
                    with ProcessPoolExecutor(max_workers=num_workers) as executor_local:
                        ...
                        for future in concurrent.futures.as_completed(futures):
                            ...
                            try:
                                ...
                                batch_results["error"] += 1
                                logger.error(f"{file_name} 처리 중 예상치 못한 오류: {str(e)}")
