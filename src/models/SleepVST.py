...
class SleepVST(nn.Module):
    def __init__(self, cfg=None, seq_len=240, d_model=128, num_classes=4, num_layers=6, num_heads=8, dropout=0.1, **kwargs):
        super().__init__()
        
        # Handle cfg or direct parameters
        if cfg is not None:
            # Extract values from cfg (could be DictConfig)
            self.seq_len = int(getattr(cfg, 'seq_len', 240))
            self.d_model = int(getattr(cfg, 'd_model', 128))
            self.num_classes = int(getattr(cfg, 'num_classes', 4))
            self.num_layers = int(getattr(cfg, 'num_layers', 6))
            self.num_heads = int(getattr(cfg, 'num_heads', 8))
            self.dropout = float(getattr(cfg, 'dropout', 0.1))
            self.input_length = int(getattr(cfg, 'input_length', kwargs.get("input_length", 150)))
        else:
            ...
            self.input_length = kwargs.get("input_length", 150)
...
class SleepVST_BW(SleepVST):
    ...
    def forward(self, x_bw):
        """
        x_bw: (B, N, 150) - breath waveform only
        """
